...
public class Calculo {
	...
	// faz com que o avião mova para outra posição
	...
	public static void rotacionar(Aviao aviao, double cordX, double cordY, double angulo) {
		...
		// caso a cordenada seja maior que 0, altera para negativo, caso contrario mantém
		double auxX = cordX * -1;
		double auxY = cordY * -1;
	...
	// calculo de distancia: sqrt faz calculo de raiz, x1-x2 * x1-x2 para representar o quadrado
	...
	public static InfoColisao calcularColisao(Aviao a, Aviao b) {
		...
		// converte a direção em radianos e depois aplica o valor na tangente, encontrando m
		...
		double tempoA = (distanciaA / a.getVelocidade()) * 3600;
		double tempoB = (distanciaB / b.getVelocidade()) * 3600;

		// caso a velocidade de A for maior que B, então velocidade de A - B, senão velocidade de B - A
		double diferencaTempo = (tempoA > tempoB) ? tempoA - tempoB : tempoB - tempoA;
		info.setDiferencaTempo(formatarDecimal(diferencaTempo));

		// se velocidade de A for maior que velocidade de B, então velocidade de B, senão A
